...
export default defineConfig(({ mode, command }) => {
  ...
  return {
    ...
    plugins: [
      ...
      react(),
      // The React Compiler needs Babel, which the React plugin above no longer
      // runs: it transforms JSX with Oxc and exposes the compiler as a preset
      // to hand to a Babel plugin of one's own. Skipped for the coverage build,
      // because the compiler rewrites components into cache reads and writes,
      // moving the statements istanbul instrumented out from under the
