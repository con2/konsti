...
- **Env files** are selected by build `--mode`: `.env.development`, `.env.staging`, `.env.production`, `.env.ci`, `.env.kube-dev`. Vite's `define` exposes config as `process.env.SETTINGS`, `process.env.API_SERVER_URL`, `process.env.SHOW_TEST_VALUES`, `process.env.DATA_UPDATE_INTERVAL`, and `process.env.APP_VERSION` (the build's git SHA from the `APP_VERSION` env/build-arg, also used as the Sentry release name; development/ci builds fall back to a fixed placeholder so E2E can drive the update banner; these are **not** `VITE_`-prefixed) — they feed `config.client()` in `shared/config`.
...
- **App update notification:** `components/AppUpdateBanner.tsx` prompts a reload when the version the server reports in the polled settings response differs from this bundle's own build SHA (`config.client().appVersion`; both come from the `APP_VERSION` build-arg, so they match only when the client runs the deployed build). A version is accepted only after polls have reported it unchanged for half a poll interval (`updateServerAppVersion` in `adminSlice`) so rolling deploys don't flash the banner. It also reloads transparently on the first route navigation after detection, at most once per version (sessionStorage guard prevents reload loops). Never shows in local dev on its own because the server reports an empty version there; the E2E spec fakes the server-reported version to drive it.
