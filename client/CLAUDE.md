...
- **Lazy loading:** `utils/lazyWithRetry.ts` wraps `React.lazy` to retry failed dynamic imports. `index.tsx` loads `App` through it; `AppRoutes.tsx` loads the About tabs (`AboutView`/`FaqView`/`InstructionsView`) and the `AdminView`/`HelperView` staff tools the same way, and `components/PrivacyNotice.tsx` defers its Markdown until the accordion opens. The route-level `Suspense` boundary sits around `<AppRoutes />` in `App.tsx`, not at the root, so a view loads with the header and sticky bars still on screen. Split content-heavy or role-restricted views; leave the critical path (login form, program list) eager.
