...
- **Prefer an explicit `return` over an expression-bodied arrow function.** Write `(x) => { return f(x); }`, not `(x) => f(x)`, for any function declared with `const`. Short inline callbacks passed straight to `filter`/`map` and the like may stay as expressions.
...
- **Never point a code comment at a `CLAUDE.md`.** Those files are guidance for working on the code, not documentation the code is entitled to lean on: a comment saying "see Error Reporting in client/CLAUDE.md" goes stale when a section is renamed, and it leaves the line explaining nothing on its own. State the constraint the reader needs right there, in a clause, and leave the rest to the `CLAUDE.md` for whoever goes looking.
