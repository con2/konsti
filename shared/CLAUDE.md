...
- **`relativeTime.ts`** — `formatRelativeTime`, hand-written rather than date-fns `formatDistance` because the two bucket differently (date-fns calls anything under 30s "less than a minute"; this reports "a few seconds" up to 45s) and the wording is user visible. `time.test.ts` pins every string in both languages.
