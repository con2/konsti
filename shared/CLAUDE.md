...
- **`signupTimes.ts`** — lottery/direct sign-up window calculations (`getLotterySignupStartTime`/`…EndTime`, `getDirectSignupStartTime`/`…EndTime`, in-progress/started/ended predicates). Applies the parent start-time override (see below). The "open the previous evening at a fixed hour" path does its arithmetic on a `TZDate` in the event timezone and names the target hour explicitly, so it lands on the same wall-clock hour whether the previous day was 23, 24 or 25 hours long; `signupTimes.test.ts` pins both transitions. A `TZDate` serializes to its own offset rather than to UTC, so it is converted back to a plain `Date` before being returned — these values are stored and compared as ISO strings elsewhere. Also owns the two "is a lottery still coming for this program item" predicates: `willNotBeLotteried`, the gate for offering a lottery at all, and `hasLotteryAlreadyRun`, the narrower "moved since its lottery" case that only the help text needs to tell apart. `hasLotteryAlreadyRun` reads `lotteryRanForStartTime`, which only a run ever writes; `willNotBeLotteried` also covers `passedOverForLottery`, which is what an import or a skipped run writes instead. They live here rather than beside `isLotterySignupProgramItem` because `getDirectSignupStarted` needs them: direct sign-up never closes again once it has been open (rule 10 in [`docs/en/lottery-design-rules.md`](../docs/en/lottery-design-rules.md)). Ask "is direct sign-up open" through `getDirectSignupStarted`/`getDirectSignupInProgress` rather than comparing against `getDirectSignupStartTime` inline — the schedule-derived time still answers the run-gating question ("has the first-come phase for this hour begun"), which is a property of the start time, not of one item let out early.
