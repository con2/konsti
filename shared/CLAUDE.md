...
- **`config/`** — application/event configuration, read everywhere through the `config` object (`shared/config/index.ts`): `config.event()`, `config.client()`, `config.server()`, `config.sentry()`.
  ...
  - `past-events/` — one exported config per past event (e.g. `ropecon2025.ts`), typed `Partial<EventConfig>`, plus an `index.ts` listing them with the datafile directory each event's dump lives in. They are archives that nothing in the app reads; the past-event lottery replay (see [server/CLAUDE.md](../server/CLAUDE.md)) is what consumes them.
- **`constants/`** — `apiEndpoints.ts` (`ApiEndpoint`, `ApiDevEndpoint`, `AuthEndpoint`), `browserStorage.ts` (`browserStoragePrefix`/`localStorageStateKey`, the event-prefixed browser storage keys shared by client and playwright — see the root CLAUDE.md's Database Lifecycle section), `signups.ts` (e.g. `DIRECT_SIGNUP_PRIORITY = 0`), `validation.ts` (username/password/message length bounds used by zod schemas), `programTypeNames.ts` (each program type's plural name in both languages). That last one is the single source for those names: the client's `programTypePlural` locale block spreads it and the server's emails read it directly, because the server writes both languages into one message and has no i18next instance. It holds **names, not translation keys**, which is what keeps it inside the no-i18next rule above.
- **`types/`**
...
- **`signupTimes.ts`** — lottery/direct sign-up window calculations (`getLotterySignupStartTime`/`…EndTime`, `getDirectSignupStartTime`/`…EndTime`, in-progress/started/ended predicates). Applies the parent start-time override (see below). The "open the previous evening at a fixed hour" path does its arithmetic on a `TZDate` in the event timezone and names the target hour explicitly, so it lands on the same wall-clock hour whether the previous day was 23, 24 or 25 hours long; `signupTimes.test.ts` pins both transitions. A `TZDate` serializes to its own offset rather than to UTC, so it is converted back to a plain `Date` before being returned — these values are stored and compared as ISO strings elsewhere. Also owns the two "is a lottery still coming for this program item" predicates: `willNotBeLotteried`, the gate for offering a lottery at all, and `hasLotteryAlreadyRun`, the narrower "moved since its lottery" case. The run needs that narrower one: a program item marked for a slot it no longer starts at must not close a start time whose own lottery has never happened, nor stretch the span a rejection names. The help text asks it to choose between two explanations for offering only direct sign-up; every other caller wants the broader question. `hasLotteryAlreadyRun` reads `lotteryRanForStartTime`, which only a run ever writes; `willNotBeLotteried` also covers `passedOverForLottery`, which is what an import or a skipped run writes instead. A third, `lotteryRanForProgramItem`, asks only whether that mark is there at all — "a lottery ran for this item, wherever it now sits" — and is what the automatic lottery sign-up removals ask beside the sign-up window, which a move to a later slot would otherwise reopen. Ask it of the **stored** program item: the incoming Kompassi programme carries no marks. They live here rather than beside `isLotterySignupProgramItem` because `getDirectSignupStarted` needs them: direct sign-up never closes again on a program item the lottery will not take (rule 9 in [`docs/en/lottery-design-rules.md`](../docs/en/lottery-design-rules.md), which records what that leaves uncovered). Ask "is direct sign-up open" through `getDirectSignupStarted`/`getDirectSignupInProgress` rather than comparing against `getDirectSignupStartTime` inline — the schedule-derived time still answers the run-gating question ("has the first-come phase for this hour begun"), which is a property of the start time, not of one item let out early.

  For a two-phase item, direct sign-up opens `phaseGap` after the lottery closes — except for the slots at the very start of the event, where it opens at `eventStartTime` with no gap. There is nothing to gap from there: lottery sign-up opens `preSignupStart` before the start time but never before the event, so for those slots its window closes at or before it opens and nobody can hold a lottery sign-up for them. A run reaching one places nobody, so opening direct sign-up the same minute takes nothing away, and "a program item is empty when it is lotteried" (see [`docs/en/lottery-design-rules.md`](../docs/en/lottery-design-rules.md)) still holds. The first start time with a usable lottery window gets its full gap.

  Two things follow. **`getPhaseGapInProgress` asks whether a lottery is coming for the program item at all** — `isLotterySignupProgramItem` and `tooEarlyForLotterySignup` beside `willNotBeLotteried` — rather than inferring it from the sign-up times lining up. Deriving it from the arithmetic reported a gap for anything whose sign-up happened to open where one would have ended: a rolling item clamped to the event start, a windowed one that far after its window opens, an early slot whose gap the clamp removed. **`tooEarlyForLotterySignup` lives here for that reason**, rather than in a file of its own — `signupTimes.ts` needs it, and it needs `getProgramItemStartTime`. Its cutoff is derived from `directSignupPhaseStart` plus the hour of slack, not hard-coded at three hours, so it stays the line the clamp draws if that config value ever changes; the two disagreeing would offer a lottery to a program item whose direct sign-up opens before it closes.
...
- **`isStartTimeChanged.ts`**, **`exhaustiveSwitchGuard.ts`** (TS exhaustiveness helper that throws on unreachable cases), **`remedaExtend.ts`** (extra collection helpers), **`formatSerial.ts`** (hyphenates registration codes for display, e.g. 012-304-800-1).
