...
- **`signupTimes.ts`** — lottery/direct sign-up window calculations (`getLotterySignupStartTime`/`…EndTime`, `getDirectSignupStartTime`/`…EndTime`, in-progress/started/ended predicates). Applies the parent start-time override (see below). The "open the previous evening at a fixed hour" path does its arithmetic on a `TZDate` in the event timezone and names the target hour explicitly, so it lands on the same wall-clock hour whether the previous day was 23, 24 or 25 hours long; `signupTimes.test.ts` pins both transitions. A `TZDate` serializes to its own offset rather than to UTC, so it is converted back to a plain `Date` before being returned — these values are stored and compared as ISO strings elsewhere. Also owns the two "is a lottery still coming for this program item" predicates: `willNotBeLotteried`, the gate for offering a lottery at all, and `hasLotteryAlreadyRun`, the narrower "moved since its lottery" case. The run needs that narrower one: a program item marked for a slot it no longer starts at must not close a start time whose own lottery has never happened, nor stretch the span a rejection names. The help text asks it to choose between two explanations for offering only direct sign-up; every other caller wants the broader question. `hasLotteryAlreadyRun` reads `lotteryRanForStartTime`, which only a run ever writes; `willNotBeLotteried` also covers `passedOverForLottery`, which is what an import or a skipped run writes instead. A third, `lotteryRanForProgramItem`, asks only whether that mark is there at all — "a lottery ran for this item, wherever it now sits" — and is what the automatic lottery sign-up removals ask beside the sign-up window, which a move to a later slot would otherwise reopen. Ask it of the **stored** program item: the incoming Kompassi programme carries no marks. They live here rather than beside `isLotterySignupProgramItem` because `getDirectSignupStarted` needs them: direct sign-up never closes again on a program item the lottery will not take (rule 9 in [`docs/en/lottery-design-rules.md`](../docs/en/lottery-design-rules.md), which records what that leaves uncovered). Ask "is direct sign-up open" through `getDirectSignupStarted`/`getDirectSignupInProgress` rather than comparing against `getDirectSignupStartTime` inline — the schedule-derived time still answers the run-gating question ("has the first-come phase for this hour begun"), which is a property of the start time, not of one item let out early.
  ...
  **`tooEarlyForLotterySignup`'s cutoff is derived from `directSignupPhaseStart` plus the hour of slack**, not hard-coded at three hours, so it stays the line the clamp draws if that config value ever changes; the two disagreeing would offer a lottery to a program item whose direct sign-up opens before it closes.
