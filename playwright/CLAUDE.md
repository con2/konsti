...
**`docker-compose:test` (`playwright/runE2e.ts`) rebuilds both images before `up`** (`docker compose build server playwright`, with `COMPOSE_BAKE=true` so Bake builds them in parallel — the ~30s server build finishes inside the ~60s playwright build). The playwright service is build-only, so a bare `docker compose up` reuses a cached image and would silently run a **stale** copy of the specs after local edits — the explicit build keeps every run on the current `tests/`/`pages/`. It's near-free on repeats (Docker layer-caches `yarn install`) and in CI (fresh runners build once anyway). Don't drop the build step.
...
**No image-build caching:** each shard builds both images itself via `docker-compose:test` (straight into the daemon). This was measured against a `docker/build-push-action` + GitHub Actions layer cache (`type=gha`) setup, which turned out **slower** (~300s vs ~190s per shard): the `docker-container` driver's image export/load round-trip and the `type=gha` cache transfer cost more than rebuilding, and `cache-to,mode=max` penalized the writer shard. The per-shard build is already fast and parallel, so there's nothing to cache-win here — don't re-add buildx/gha caching.
...
A worktree's own `node_modules` must be **complete** — the same "Invalid hook call" / duplicate-React crash (breaking every page, so every spec times out on the first locator) also appears when a dependency is simply **missing** from `<worktree>/node_modules`: because the worktree is nested under the main checkout, Node/Vite then resolves that package from the **parent** checkout's `node_modules`, loading a second React copy. A partial `yarn install` in a fresh worktree is the usual culprit (e.g. `@tanstack/react-virtual` absent locally while `react` is present). Distinguish it from the stale-zombie cause above by resolution path — check `node -e "console.log(require.resolve('@tanstack/react-virtual'))"`; if it points outside the worktree, re-run `yarn install`, clear `node_modules/.vite`, and restart the client.
...
- **`populateDb(request, options)`** — `POST /api/populate-db` to seed users/admin/program-items/sign-ups/event-log (the dev-only endpoint; see [server/CLAUDE.md](../server/CLAUDE.md)).
...
- **`postSettings(request, settings)`** / **`postTestSettings(request, settings)`** — admin-authenticated `POST /api/settings` and `POST /api/test-settings`. `postTestSettings({ testTime })` **mocks server time**, which governs sign-up windows and whether the lottery has "run".
- **`signupsOpenTime()`** / **`hoursIntoEvent(hours)`** — the suite's whole time model. `signupsOpenTime()` is the moment specs treat as the start of the event: `eventStartTime`, unless the event config holds direct sign-up back past it with `rollingDirectSignupEarliestStartTime`, in which case it is that. A spec sets its mocked clock to it (`postTestSettings({ testTime: signupsOpenTime() })`) and builds program item start times with `hoursIntoEvent(n)`, which counts whole hours from the same instant — **never from `config.event().eventStartTime` directly**, or the item times and the clock drift apart when an event configures the two differently. An offset of 1-4 hours puts a rolling program item's direct sign-up open at that clock; a two-phase item's does not open until `directSignupPhaseStart` before it starts, so only `hoursIntoEvent(1)` is ever in its direct sign-up phase there.
- **`testPostDirectSignup(request, username, req)`** — direct-signup as another user (e.g. to fill a slot before the user under test).
- **`postAssignment(request, assignmentTime)`** — trigger the lottery for a given starting time. The server refuses a run outside that starting time's own window (lottery sign-up still open, or direct sign-up already open), so the helper moves the mocked clock to the moment lottery sign-up closes, runs, and puts the clock back where the test had it — a spec sets `testTime` for what it is asserting and doesn't have to name the lottery's moment as well.
...
- **`pauseClock(page)`** — `clock.pauseAt(...)`, so timers fire only on explicit jumps. An installed clock otherwise keeps advancing in real time, and a slow CI step can then push a timed decision past the window it belongs to or fire a straggler timer mid-assertion. Call it before any `fastForward`: the pause target must sit ahead of the page's mocked clock (pausing into the past throws).
- **`fastForwardUntilVisible(page, locator, step?)`** — jumps the clock repeatedly (default `"00:05"`) until the locator is visible, inside a 30 s `toPass`. Small steps let fetch rejections settle on the real event loop and schedule their follow-up timers between jumps. Pass a step of a full poll interval (`"01:01"`) when what's awaited needs a fresh **load** rather than only a pending timer — the app drops an interval tick that arrives while the previous load is still in flight, so a shorter step can burn the whole budget waiting for the tick after it.

## Page visibility helper (`pageVisibilityUtils.ts`)

Playwright has no visibility emulation, and every spec's `page` is the foregrounded sole page of its context, so `document.hidden` is always `false` unless a spec says otherwise. **`setPageHidden(page, hidden)`** overrides `document.hidden`/`visibilityState` in the page and dispatches `visibilitychange`, which is what the app's page-resume detection listens to; the override lasts until the next navigation. Two things follow from the app's hidden-page rules: the data poll skips its ticks while hidden and refreshes on show only when its last good load is older than a poll interval, and the network-error toast is suppressed while hidden - so a spec that fails requests while "hidden" is testing a different toast path than one that doesn't.

## Tests (`playwright/tests/`)

Specs cover the main user flows: auth (`login`, `kompassiLogin`, `registration`, `logout`), sign-up (`lotterySignup`, `directSignup`, `preConventionWeekSignup`, `favorite`, `admission`, `myProgramItems` — the My Program lists' cancel/unfavorite actions, empty states, and the persisted upcoming/all starting-time toggle), groups (`group`, `groupDirectSignup`), discovery (`programSearch`, `programTagFilter` — the multi-select tag filter's OR logic and sessionStorage persistence, `programListFilters` — the hide-full-items checkbox, the revolving door starting-time view, and persistence of all the filter card's selections (program type, starting time, search term, hide-full) over a reload, `programListQueryParams` — the `programType`/`invalid` program-list URL params, `programItemErrors` — the per-card error messages for items missing required info (attendance limits, sign-up type), `programListScroll` — the virtualized list's scroll restore/reset on navigation, `programDateDisplay` — program times show full dates outside event week and weekday-only during it), staff tools (`admin`, `adminConsole`, `helper`), `profile`, `about`, `dashboard` — the public `/dashboard` lottery-results view (no login required), program-item lifecycle (`cancelProgramItem`, `moveProgramItem` — the cancellation/start-time-change notifications described in [server/CLAUDE.md](../server/CLAUDE.md)), `dataPolling` — the app's periodic background data refresh (`dataUpdateInterval`, 60 s) delivering server-side changes without navigation, its ticks skipped while the page is hidden, and the refresh on show that happens only when the data went stale meanwhile, `appUpdateBanner` - the new-version banner (patches the polled settings response to fake the server-reported version: confirmation-window acceptance, dismiss, the reload button, and the transparent once-per-version reload on the first navigation), `appLevelBars` — how the bars below the header sit together rather than what any one of them does (sticky stacking without overlap, and the dismiss icons sharing a right edge and vertical centre); each bar's own behaviour is covered where its feature lives, and `networkError` — the network-error toast state machine (toast on outage after the decision delay, healing on the next successful response, grace-period suppression of a failing refresh right after reconnect, the probe surfacing a genuine outage, HTTP errors on background requests counting as connectivity issues inside the grace period and as API errors outside it, immediate toasts for user-initiated failures, the profile email form reporting a failed follow-up session refresh instead of success, and the password form reporting a failed change instead of success). These use `page.clock.install()` + `fastForward()` to trigger timers instantly — install the mocked clock **before** `page.goto` so the app's `setInterval` uses it; promise/fetch resolution is unaffected, only timers. `networkError` simulates network failure with `page.route(...)` + `route.abort()` (not `context.setOffline`, which would flip `navigator.onLine` and route the client into its offline branch), drives the mocked clock through `clockTestUtils.ts` (see above), and dispatches a synthetic `online` event to stamp the reconnect grace period (the page-resume path into the same grace period is drivable with `setPageHidden`, see above, but isn't exercised there).

**Any spec that waits for the network-error toast must abort _every_ API request, not just the one it cares about.** The client heals the toast on any successful response (`onRequestSuccess` in `client/src/utils/networkErrorPolicy.ts`), and the app's data load runs its requests sequentially, so leaving one endpoint working means the next poll removes the toast again — often between the assertion that it appeared and the measurement that follows. `appLevelBars` failed this way in the Coverage workflow, where the instrumented dev server is slow enough for the race to land.
...
- Components: `Navigation`, `NotificationBar`, `ErrorBar` (the app-wide error toast bar; toasts are dismissed with their close button, like the other app-level bars), `AppUpdateBanner` (the new-version notification banner with its reload and dismiss buttons), and `ProgramItemCard` (wraps a single `program-item-container`; `ProgramListPage.firstItem()` / `itemByTitle()` return one).
- Pages: `LoginPage`, `RegistrationPage`, `ProgramListPage` (both tabs plus the My Program lists), `ProgramItemPage`, `ProfilePage`, `GroupPage`, `AdminPage`, `HelperPage`, `AboutPage`, `DashboardPage`.
...
- **One `page.goto` per test where you can.** Loading the app and then navigating away mid-boot makes the abandoned page reload itself, and on WebKit that reload interrupts the second navigation (`page.goto: Navigation to "X" is interrupted by another navigation to "Y"`, or `Frame load interrupted`). It surfaces in the **Coverage** workflow rather than `test.yml`: `coverage:e2e` serves the app from the istanbul-instrumented Vite dev server, where booting is slow enough for the race to land, while the `e2e` job runs the production build out of the server container. So set up browser storage with `page.addInitScript` (`kompassiLogin`, `logout`, `programSearch` all do) instead of a throwaway `goto` plus `page.evaluate` — the same reason `login()` registers an init script rather than navigating.
...
- **Two "Save" buttons:** in Profile / `PasswordChangeForm` the email Save is `nth(0)` and the password Save is `nth(1)` — use `ProfilePage.saveEmail()` / `savePassword()` (and `HelperPage.changePassword()` for the helper view's password Save).
...
- **Kompassi login E2E needs the mock:** it only passes when the server resolves Kompassi to the bundled mock (`SETTINGS=ci`, or `KOMPASSI_BASE_URL` pointed at the local server). Under `yarn playwright` (`SETTINGS=development` with a real `KOMPASSI_BASE_URL`) the OIDC step redirects to the real Kompassi and the test can't complete — run `kompassiLogin` via `yarn docker-compose:test`.
