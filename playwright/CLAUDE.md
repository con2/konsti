...
Specs cover the main user flows: auth (`login`, `kompassiLogin`, `registration`, `logout`), signup (`lotterySignup`, `directSignup`, `preConventionWeekSignup`, `favorite`, `admission`, `myProgramItems` — the My Program lists' cancel/unfavorite actions and empty states), groups (`group`, `groupDirectSignup`), discovery (`programSearch`, `programTagFilter` — the multi-select tag filter's OR logic and sessionStorage persistence, `programListQueryParams` — the `programType`/`invalid` program-list URL params, `programListScroll` — the virtualized list's scroll restore/reset on navigation, `programDateDisplay` — program times show full dates outside event week and weekday-only during it), staff tools (`admin`, `adminConsole`, `helper`), `profile`, `about`, program-item lifecycle (`cancelProgramItem`, `moveProgramItem` — the cancellation/start-time-change notifications described in [server/CLAUDE.md](../server/CLAUDE.md)), and `dataPolling` — the app's periodic background data refresh (`dataUpdateInterval`, 60 s) delivering server-side changes without navigation. `dataPolling` uses `page.clock.install()` + `fastForward()` to trigger the poll instantly — install the mocked clock **before** `page.goto` so the app's `setInterval` uses it; promise/fetch resolution is unaffected, only timers.
