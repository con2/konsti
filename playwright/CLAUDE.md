...
- Components: `Navigation`, `NotificationBar`, `ErrorBar` (the app-wide error toast bar; toasts are dismissed by clicking them), and `ProgramItemCard` (wraps a single `program-item-container`; `ProgramListPage.firstItem()` / `itemByTitle()` return one).
