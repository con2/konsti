...
- **`twoPhaseProgramItem(startTime, overrides?)`** / **`endTimeFor(startTime)`** — the fixture as a lottery program item at a start time, its end following from the fixture length; pass overrides for attendance limits, tags or state. Use it for the common case instead of spreading `testProgramItem` by hand.
- **`helsinkiClockTime(time)`** — the `HH:mm` the app should render an instant at, built with `Intl` rather than the app's own formatters so that losing the event timezone fails the assertion instead of moving both sides of it together.
...
- **`PAST_POLL_TICK`** — the `"01:01"` jump that carries the clock past the app's 60 s data poll; specs that wait for a fresh load jump by it rather than by a literal.
