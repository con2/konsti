...
export enum KompassiTopicTracon {
  BOFFERIN = "boffering",
...
export enum KompassiKonstiProgramTypeTracon {
...
export enum KompassiAudienceTracon {
...
export enum KompassiPlaystyleTracon {
  SERIOUS = "serious",
  LIGHT = "light",
...
export enum KompassiLanguageTracon {
  FINNISH = "fi",
  ENGLISH = "en",
  SWEDISH = "sv",
  LANGUAGE_FREE = "free",
...
export enum KompassiAccessibilityTracon {
...
export const KompassiProgramItemSchemaTracon = z.object({
  slug: z.string(),
  ...
  cachedDimensions: z.object({
    ...
    topic: z.array(z.nativeEnum(KompassiTopicTracon)).catch((ctx) => {
      if (!Array.isArray(ctx.input)) {
        return [];
      }
      const [valid, invalid] = partition(ctx.input, (topic) =>
        Object.values(KompassiTopicTracon).includes(topic),
      );
      logger.error(
        "%s",
    ...
    konsti: z.array(z.nativeEnum(KompassiKonstiProgramTypeTracon)).catch([]),
    audience: z.array(z.nativeEnum(KompassiAudienceTracon)).catch((ctx) => {
      if (!Array.isArray(ctx.input)) {
        return [];
      }
      const [valid, invalid] = partition(ctx.input, (audience) =>
        Object.values(KompassiAudienceTracon).includes(audience),
      );
      logger.error(
        "%s",
    ...
    language: z.array(z.nativeEnum(KompassiLanguageTracon)).catch((ctx) => {
      if (!Array.isArray(ctx.input)) {
        return [];
      }
      const [valid, invalid] = partition(ctx.input, (language) =>
        Object.values(KompassiLanguageTracon).includes(language),
      );
      logger.error(
        "%s",
    ...
    accessibility: z
      .array(z.nativeEnum(KompassiAccessibilityTracon))
      .catch((ctx) => {
        if (!Array.isArray(ctx.input)) {
          return [];
        }
        const [valid, invalid] = partition(ctx.input, (accessibility) =>
          Object.values(KompassiAccessibilityTracon).includes(accessibility),
        );
        logger.error(
          "%s",
    ...
    playstyle: z.array(z.nativeEnum(KompassiPlaystyleTracon)).catch((ctx) => {
      if (!Array.isArray(ctx.input)) {
        return [];
      }
      const [valid, invalid] = partition(ctx.input, (playstyle) =>
        Object.values(KompassiPlaystyleTracon).includes(playstyle),
      );
      logger.error(
        "%s",
...
export type KompassiProgramItemTracon = z.infer<
