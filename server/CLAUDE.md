...
| `test` / `test:watch` / `test:coverage`                                                               | Vitest unit/integration tests against `mongodb-memory-server`                                                                     |
...
- **`index.ts`** — entry point (ESM). Imports `utils/instrument.ts` first so Sentry is initialized before anything else.
...
- **`utils/`** — cross-cutting helpers: `server.ts` (Express app assembly), `logger.ts` (winston), `instrument.ts` (Sentry), `cron.ts` (scheduled jobs), `notificationQueue.ts` (email queue), `jwt.ts`, `bcrypt.ts`, `authHeader.ts`, `zodUtils.ts`, etc.
