...
- **Email notifications:** queued through `utils/notificationQueue.ts` (a `fastq` queue) and sent via `nodemailer`. `EmailSender` takes an `EmailDelivery` mode, and **only production puts queued mail on the network**: outside it the default `QUEUED` mode uses `jsonTransport`, because the non-production SMTP host is a shared free service that rate-limits a lottery run's worth of notifications wholesale (each rejection then logs an error, so one run floods Sentry) and staging's addresses are generated ones. The admin "send test email" button passes `ADMIN_TEST` — one message, deliberately asked for — which still goes out and returns a preview URL. Don't hand the queue a delivering transport to make staging mail arrive.
