...
- `logApiCall` (mounted on `apiRoutes` only, not `sentryRoutes`) writes one access line per request via winston on response finish: `API call: METHOD /path STATUS Xms user=X ip=Y size=Z`. Skips OPTIONS preflights and strips `::ffff:` from IPv4-mapped IPs. Replaces morgan, which has been removed. It also **skips anything outside `/api` and `/auth`**: router-level middleware on a router mounted at the app root runs before any of that router's routes match, so it sees the static assets, `index.html` and SPA-fallback requests that fall through to the static middleware below it too - a single page load fetches dozens of chunks, which buried the API calls in the deployed logs.
...
- **`getHealthStatus`** lives in `server/src/api/healthRoutes.ts` (separate router) because it is the **only** route a cronjob-only instance serves. `onlyCronjobs` gates `apiRoutes`, `sentryRoutes` and the static SPA together, so the cron deployment can neither serve nor mutate attendee data; its k8s liveness and readiness probes hit `/api/health`, which would restart the pod in a loop if that route went with the rest. It carries `allowCORS` as **route-level** middleware rather than `router.use(...)`: the router is mounted at the app root, where a router-level `use` runs for every request the app receives, not just this route's — which is how `logApiCall` once came to log each API call twice, and how CORS would reach the paths `onlyCronjobs` is there to close off. The route deliberately carries **no** `logApiCall`: the liveness and readiness probes hit it every few seconds, so logging them adds thousands of lines a day and no signal (a failing probe surfaces as a pod restart). CORS is needed here at all because the client's network probe fetches `/api/health` cross-origin whenever the client is served from its own dev server, and without the header the browser drops the response. Add new routes to `apiRoutes` unless the cron instance itself needs them.
...
**Assignment test organization** (`run-assignment/`): put generic, algorithm-independent behavior — start-time filtering, sign-up cleanup/preservation, result snapshots, error cases — in `runAssignment.test.ts`. Behaviour every algorithm must share runs there once per algorithm through a `describe.each`. A per-algorithm file (`runAssignmentRandomPadg.test.ts` is the only one so far) holds only cases specific to that algorithm, such as its fallback between the two. New generic cases go in `runAssignment.test.ts`.
