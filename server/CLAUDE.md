...
- **Security & static serving:** `helmet` for headers; the SPA in `server/front/` is served with `express-static-gzip` (brotli/gzip). Cache policy: content-hashed files under `assets/` get a one-year immutable `Cache-Control` while `index.html` gets `no-cache`, so clients revalidate the HTML on every load but keep old chunks cached across deploys (a stale page still finds its matching chunks instead of fetching deleted files).
